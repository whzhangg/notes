...
$k$ is a positive scaling constants. 

If we take $k = 1$, and take $\log_2$ for logarithm, then
the information content is then measured in bits. As an example, consider the 
...
to store whether the result is face up.

To describe a probability distribution, we can use a series of statements. For example, 
to describe the probability distribution of weather (sunny, cloudy and rainy) in a day, we 
have a series of statements:
\begin{itemize}
...
\end{equation}
information content $S$ is called \textbf{Shannon entropy}. We can consider Shannon entropy 
as measuring how much information we can gain by performing a measurement on the system.

Consider a Bernoulli trial with outcome probability $p$. Using $k=1$, the 
Shannon entropy is calculated by:
\begin{equation}
    S = - \sum_i P_i \log_2 P_i = -p \log_2 p - (1-p) \log_2 (1-p)
\end{equation}
$S$ is maximum when $p = 0.5$, showing that the information content of a distribution is greatest
when the uncertainty of output is greatest: we gain the most information when we can determine 
the outcome.
When $p$ deviates from $0.5$, the outcome become biased and it is very likely that most measurement
give the same result, then what we gain from measuring the outcome is relatively less important and 
the information entropy contained in this distribution is little. In the extreme, if the event is 
deterministic, than we will gain no information after measurement of the result, thus the information 
contained any statement regarding to the result is simply $0$.
