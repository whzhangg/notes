...
- when we do not need to build the forward graph, we should also use `torch.no_grad()` context, for example, when evaulating the validation loss. This will reduce the memory consumption and improve speed.
